...
Componentes React do **Industry** — o design system de blueprint sobre fundo escuro — consumindo tokens de [`@industry/tokens`](../tokens/README.md). Espelha a estrutura e o tooling do extinto `@vuotto/web` (removido do monorepo, REB-100): export único `"."`, `dist/` construído com `tsup`, Storybook em `@storybook/react-vite`.
