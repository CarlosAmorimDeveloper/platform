...
Componentes React Native do **Industry** — consumindo tokens de [`@industry/tokens`](../tokens/README.md). Espelha a estrutura e o tooling do extinto `@vuotto/mobile` (removido do monorepo, REB-100): export único `"."`, `dist/` construído com `tsup`, Storybook via `react-native-web`. Diferente de `@vuotto/mobile`, tem um setup de Jest (porta do que o extinto `@ds/mobile` usava), sem a entrada de `react-native-paper` — este pacote não depende dele).
